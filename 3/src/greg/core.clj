...
(defn make-direction-vector
  [direction-string]
  (cond
    (clojure.string/starts-with? direction-string "R") [(parse-int direction-string) 0]
    (clojure.string/starts-with? direction-string "L") [(* -1 (parse-int direction-string)) 0]
    (clojure.string/starts-with? direction-string "U") [0 (parse-int direction-string)]
    (clojure.string/starts-with? direction-string "D") [0 (* -1 (parse-int direction-string))]))

(defn make-vector-point
  [[x1 y1] [x2 y2]]
  [(+ x1 x2) (+ y1 y2)])
  ...
  (let [p  (first pr)
    ...
    (when (not= 0 rs)
      (let [t (/ (cross-product (vec-sub q p) s)
        ...
        (when (and (<= 0 t 1)
                   (<= 0 u 1))
          (let [result (vec-add p (vec-mul t r))]
            (mapv int result)))))))
